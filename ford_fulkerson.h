...
int find_new_flow(const int* path, const AdjacencyMatrix* residual_capacity, int numNodes, int source, int sink, bool print_progress=false){
	...
	while (current_node != sink){
		...
		edge_weight = residual_capacity->get_weight(current_node, path[i+1]); 	// YOUR CODE HERE - EDIT THIS LINE
...
int ford_fulkerson(const AdjacencyMatrix* capacity, int numNodes, int source, int sink, bool print_progress=false){
	...
	int* next_path = find_path(&residual_capacity, numNodes, source, sink); 		// YOUR CODE HERE - EDIT THIS LINE
	...
	while (next_path != NULL){							
		...
		new_flow = find_new_flow(next_path, &residual_capacity, numNodes, source, sink); 		// YOUR CODE HERE - EDIT THIS LINE

		// TO DO: add new flow to max flow
		max_flow = new_flow + max_flow;
		...
		while (current_node != sink){		// repeat until we reach the sink
			...
			// YOUR CODE HERE
			flow.set_weight(current_node, next_node, flow.get_weight(current_node, next_node) + new_flow);
			flow.set_weight(next_node, current_node, flow.get_weight(current_node, next_node) - new_flow);
		...
		next_path = find_path(&residual_capacity, numNodes, source, sink); 		// YOUR CODE HERE - EDIT THIS LINE		
	}
